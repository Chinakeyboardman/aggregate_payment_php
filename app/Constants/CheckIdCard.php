...
class CheckIdCard
{
    ...
    function get_address($idcard, $type = 1)
    {
        ...
        switch ($type) {
            case 1:
                ...
                if (!empty(self::$Addr_arr[$key])) return self::$Addr_arr[$key];
    ...
    public function getStarSign($cid)
    { // 根据身份证号，自动返回对应的星座
    ...
    public function getAnimalSign($cid)
    { //根据身份证号，自动返回对应的生肖
        if (!$this->isIdCard($cid)) return '';
        $start = 1901;
    ...
    public function getSexSign($cid)
    { //根据身份证号，自动返回性别
        if (!$this->isIdCard($cid)) return '';
        $sexInt = (int)substr($cid, 16, 1);
        return $sexInt % 2 === 0 ? 2 : 1; // 女   ： 男
    }
    ...
    { // 检查是否是身份证号
        ...
        //加权因子
        $wi = array(7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2);  //校验码串
        $ai = array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2');
        //按顺序循环处理前17位
        $sigma = 0;
        for ($i = 0; $i < 17; $i++) {//提取前17位的其中一位，并将变量类型转为实数
            if (!isset($number[$i])) {
                return false;
            }
            $b = (int)$number[$i];
    ...
    {
        if (!self::$GetInfoByIDCard) {
            self::$GetInfoByIDCard = new GetInfoByIDCard();
        }
        return self::$GetInfoByIDCard;
    }
}